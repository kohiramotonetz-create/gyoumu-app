...
- 日付: 2026-08-27
- 状態: React・GAS実装、ローカル自動検証、対象GASプロジェクトへのclasp pushを完了。専用シートの本番セットアップ、Webアプリ再デプロイ、実動作確認は未実施。
...
- 実画面改善: `lessonDate`を日本時間の暦日文字列としてAPI返却しUTC変換による前日ずれを防止。進捗ラインは章境界と現在単元名を強調し、履歴は日本語日付、学校の連続区間、ネッツの実施単元、VOID監査表示を講師向けに整理する。
