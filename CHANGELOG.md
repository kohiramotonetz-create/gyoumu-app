...
- セットアップ: `setupAcademicResultSheets()`を追加。`学校成績テスト`はgradeを持たない10列を正式仕様とし、旧11列は自動変更せず想定外ヘッダーとして停止する。旧11列を作成済みの場合は、年度内同名テストがないことを確認してからユーザーがC列gradeを削除する。Codexによるsetup実行、本番シート変更、GAS Webアプリ再デプロイ、本番成績入力、実環境確認は未実施。
