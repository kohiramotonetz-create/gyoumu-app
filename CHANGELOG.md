...
- スキマ君: gyoumu-app GASがログSpreadsheetを読み、正式`contentId`がある行だけをuserIdと組み合わせて集計する。contentId空欄の旧ログは推測統合せず件数だけを注記し、現在許可コンテンツと権限OFF後の過去履歴を分けて返す。
- UI: 各セクションを独立loading・error・再試行にし、1対1進捗は既存一覧と共通コンポーネントを使用。1対1進捗、個トレ、学校進捗、スキマ君利用設定、アカウント管理の生徒名を共通プロフィールリンクへ変更した。
- デプロイ: clasp push、GAS Webアプリ再デプロイ、本番確認は未実施。Versionは`4.3.0`を維持。
