...
- 状態: React・GAS実装とローカル検証を実施。GAS Webアプリ再デプロイと本番実動作確認は未実施。
...
- 認証・権限: adminは全校舎、teacher・head-teacherは有効な担当校舎の有効studentだけを閲覧可能。student、担当外、無効・削除済み生徒をGAS側で拒否し、プロフィールactionごとに管理セッションと対象生徒を再検証する。
...
- UI: 各セクションを独立loading・error・再試行にし、1対1進捗は既存一覧と共通コンポーネントを使用。個トレも`units.csv`の教材別単元軸・章境界・最大到達位置を同系統の進捗ラインで表示した。PCは1対1＋個トレの2カラム、スキマ君の全幅responsive grid、学校成績表＋選択テスト棒グラフの2カラムへ整理し、狭い画面では1カラムへ切り替える。1対1進捗、個トレ、学校進捗、スキマ君利用設定、アカウント管理の生徒名を共通プロフィールリンクへ変更した。
...
- デプロイ: 初期実装のclasp pushは実施済み。今回の表示仕様変更を含むGAS Webアプリ再デプロイと本番確認は未実施。Versionは`4.3.0`を維持。
...
- 認証・権限: teacherにも期限付き管理セッションを発行するが、既存admin専用actionは引き続き`requireAdminSession()`で保護する。Issue #017はteacher・head-teacher・adminが担当範囲を閲覧・登録でき、履歴のVOID化・訂正はadminだけに限定する。受講科目と担当校舎をGAS側でも検証する。
