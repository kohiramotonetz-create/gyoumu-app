...
- UI: 各セクションを独立loading・error・再試行にし、1対1進捗は既存一覧と共通コンポーネントを使用。個トレも`units.csv`の教材別単元軸・章境界・最大到達位置を同系統の進捗ラインで表示し、学校成績の9科目表の下へテスト別棒グラフを追加した。1対1進捗、個トレ、学校進捗、スキマ君利用設定、アカウント管理の生徒名を共通プロフィールリンクへ変更した。
