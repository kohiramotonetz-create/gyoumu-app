...
import { getChapterSegments, selectChapterSegments } from '../../utils/oneToOneProgressDisplay.js';

export default function ProgressAxisLine({ axis = [], currentOrder = 0, label, color, formatUnit, renderCurrent, onUnitClick, maxChapterLabels }) {
  const [detailUnit, setDetailUnit] = useState(null);
  const currentUnit = axis.find(unit => unit.unitOrder === currentOrder);
  const segments = selectChapterSegments(getChapterSegments(axis), maxChapterLabels);
