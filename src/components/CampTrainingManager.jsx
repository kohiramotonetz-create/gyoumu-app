...
export default function CampTrainingManager({ GAS_URL, API_KEY, sessionToken, role, assignedSchools = [], styles, onSessionExpired }) {
  ...
  return <section className="camp-training-manager">
    ...
    <div className="camp-training-tabs"><button disabled={saving || inputDirty} className={view === 'ranking' ? 'active' : ''} onClick={() => changeView('ranking')}>ランキング</button>{['head-teacher', 'general', 'admin'].includes(role) && <button disabled={saving || inputDirty} className={view === 'participants' ? 'active' : ''} onClick={() => changeView('participants')}>参加者設定</button>}{['head-teacher', 'general', 'admin'].includes(role) && <button disabled={saving || inputDirty} className={view === 'input' ? 'active' : ''} onClick={() => changeView('input')}>データ入力</button>}</div>
    ...
    {view === 'participants' && ['head-teacher', 'general', 'admin'].includes(role) && <><div className="camp-participant-filters">
