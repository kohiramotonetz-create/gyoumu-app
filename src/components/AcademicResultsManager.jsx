...
import { ACADEMIC_SUBJECTS, TEST_TYPE_LABELS, areAcademicScoresEqual, calculateAcademicTotal, getAcademicYearOptions, getCurrentSchoolYear, normalizeAcademicScore, parseAcademicScorePaste } from '../utils/academicResults.js';
...
export default function AcademicResultsManager({ GAS_URL, API_KEY, sessionToken, assignedSchools = [], styles, onSessionExpired }) {
  ...
  const years = useMemo(() => getAcademicYearOptions(tests.map(test => test.schoolYear)), [tests]);
  ...
  const header = { padding: 8, minWidth: 82, background: '#f1f5f9', borderBottom: '2px solid #cbd5e1', whiteSpace: 'nowrap' };
  const fieldControl = { ...styles.select, width: '100%', minWidth: 0, minHeight: 40, boxSizing: 'border-box' };
  const fieldLabel = { display: 'flex', flexDirection: 'column', gap: 6, minWidth: 0, fontSize: 13, fontWeight: 'bold', color: '#475569' };
  const filterGrid = { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 14, alignItems: 'end' };
  return <div style={{ padding: 10 }}>
    ...
    {tab === 'tests' ? <div style={{ display: 'grid', gap: 16 }}>
      <form onSubmit={saveTest} style={panel}>
        <div style={filterGrid}>
          <label style={fieldLabel}>年度<select style={fieldControl} value={year} disabled={Boolean(editingTestId)} onChange={e => setYear(Number(e.target.value))}>{years.map(value => <option key={value} value={value}>{value}年度</option>)}</select></label>
          <label style={fieldLabel}>テスト名<input style={fieldControl} value={form.testName} onChange={e => setForm(value => ({ ...value, testName: e.target.value }))} required /></label>
          <label style={fieldLabel}>種別<select style={fieldControl} value={form.testType} onChange={e => setForm(value => ({ ...value, testType: e.target.value }))}>{Object.entries(TEST_TYPE_LABELS).map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select></label>
          <label style={fieldLabel}>満点<input style={fieldControl} type="number" min="1" max="1000" value={form.maxScore} onChange={e => setForm(value => ({ ...value, maxScore: e.target.value }))} required /></label>
        </div>
        <button disabled={saving} style={{ ...styles.doneBtn, marginTop: 14 }}>{editingTestId ? '更新' : 'テスト追加'}</button>
